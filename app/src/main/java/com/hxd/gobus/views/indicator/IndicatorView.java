 ...
 * 基于ViewPagerd用户指引(欢迎见面),广告图片及自动轮播图片(类似于Gallery效果)
 */
public class IndicatorView extends RelativeLayout {
	...
	private void init(Context context) {
		LayoutInflater.from(context).inflate(R.layout.indicatorview_layout, this);
		this.viewPager = (ViewPager) findViewById(R.id.vp_indicator);
		this.indicateLayout = (LinearLayout) findViewById(R.id.indicator_layout);
	...
	protected void refreshIndicateView() {
		...
		for (int index = 0; index < totelCount; index++) {
			...
			if (this.currentIndex == index) {
				imageView
						.setBackgroundResource(R.mipmap.indicatorview_image_indicator_focus);
			} else {
				imageView
						.setBackgroundResource(R.mipmap.indicatorview_image_indicator);
			}
		}
...
class ScrollIndicateHandler extends Handler {
	private IndicatorView scrollIndicateView;

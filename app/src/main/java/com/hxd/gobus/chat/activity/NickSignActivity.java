...
public class NickSignActivity extends JBaseActivity {
    ...
    private static final int GROUP_DESC = 50;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.getFlags() == com.hxd.gobus.chat.activity.PersonalActivity.FLAGS_SIGN) {
            initViewSign("个性签名", SIGN_COUNT);
            initData(SIGN_COUNT);
        } else if (intent.getFlags() == com.hxd.gobus.chat.activity.PersonalActivity.FLAGS_NICK) {
    ...
    private void initListener(final int flags) {
        tvRightTitle.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (flags == com.hxd.gobus.chat.activity.PersonalActivity.FLAGS_NICK) {//3
                    intent.putExtra(com.hxd.gobus.chat.activity.PersonalActivity.NICK_NAME_KEY, sign);
                    setResult(com.hxd.gobus.chat.activity.PersonalActivity.NICK_NAME, intent);//4
                } else if (flags == com.hxd.gobus.chat.activity.PersonalActivity.FLAGS_SIGN) {//2
                    intent.putExtra(com.hxd.gobus.chat.activity.PersonalActivity.SIGN_KEY, sign);
                    setResult(com.hxd.gobus.chat.activity.PersonalActivity.SIGN, intent);//1
    ...
    private void initData(final int countNum) {
        mEd_sign.addTextChangedListener(new TextWatcher() {
            ...
            //    input = s.toString().substring(start).getBytes().length;
                input = s.toString().length();
            }

            @Override
            public void afterTextChanged(Editable s) {
            //    int num = countNum - s.toString().getBytes().length;
                int num = countNum - s.toString().length();
    ...
    //    int length = mEd_sign.getText().toString().getBytes().length;
        int length = mEd_sign.getText().toString().length();
    ...
    public static class MyLengthFilter implements InputFilter {
        ...
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest,
                                   int dstart, int dend) {
            int keep = mMax - (dest.toString().length() - (dend - dstart));
            if (keep <= 0) {
                return "";
            } else if (keep >= source.toString().length()) {

...
public class SharePreferenceManager {
    ...
    private static final String KEY_USERINFO_USERNAME = "user_cached_username";
    private static final String KEY_USERINFO_PASSWORD = "user_cached_password";
    private static final String KEY_USERINFO_USERINFO = "user_cached_info";

    private static final String KEY_REFRESH_TIME = "refresh_time";

    public static void setRefreshTime(long refreshTime) {
        if (null != sp) {
            sp.edit().putLong(KEY_REFRESH_TIME, refreshTime).apply();
        }
    }
    public static long getRefreshTime() {
        if (null != sp) {
            return sp.getLong(KEY_REFRESH_TIME,0);
        }
        return 0;
    }
    ...
    public static void setLoginUsername(String username) {
        if (null != sp) {
            sp.edit().putString(KEY_USERINFO_USERNAME, username).apply();
        }
    }
    public static String getLoginUsername() {
        if (null != sp) {
            return sp.getString(KEY_USERINFO_USERNAME, null);
        }
        return null;
    }
    public static void setLoginPassword(String password) {
        if (null != sp) {
            sp.edit().putString(KEY_USERINFO_PASSWORD, password).apply();
        }
    }
    public static String getLoginPassword() {
        if (null != sp) {
            return sp.getString(KEY_USERINFO_PASSWORD, null);
        }
        return null;
    }
    public static void setLoginUserInfo(String userInfoStr) {
        if (null != sp) {
            sp.edit().putString(KEY_USERINFO_USERINFO, userInfoStr).apply();
        }
    }

    public static String getLoginUserInfo() {
        if (null != sp) {
            return sp.getString(KEY_USERINFO_USERINFO, null);
        }
        return null;
    }

...
import com.hxd.gobus.chat.pickerimage.utils.AttachmentStore;
...
import com.hxd.gobus.chat.view.ImgBrowserViewPager;
import com.hxd.gobus.chat.view.PhotoView;
...
//用于浏览图片
public class BrowserViewPagerActivity extends com.hxd.gobus.chat.activity.JBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(BaseConfig.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(BaseConfig.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(BaseConfig.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(BaseConfig.POSITION, 0);
                    ...
                    public void onClick(View v) {
                        switch (v.getId()) {
                            ...
                            case R.id.jmui_top_conv_ll://转发
                                Intent intent = new Intent(BrowserViewPagerActivity.this, com.hxd.gobus.chat.activity.ForwardMsgActivity.class);
                                BaseConfig.forwardMsg.clear();
                                BaseConfig.forwardMsg.add(mMsg);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(BaseConfig.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(BaseConfig.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(BaseConfig.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(BaseConfig.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        Intent intent = new Intent();
                        intent.putExtra(BaseConfig.MsgIDs, activity.mMsgIds);
                        activity.setResult(BaseConfig.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度

 ...
 */
@ActivityScope
public class MaintainApprovalModel extends BaseModel implements IMaintainApprovalContract.Model {

    @Inject
    public MaintainApprovalModel(){
    ...
    @Override
    public Observable<String> queryMaintainApprovalData(Integer datumDataId, String datumType) {
        return RxRetrofitClient
                .create()
                .url(Constant.QUERY_UPHOLD_CAR_APPROVAL)
                .raw(HttpParamsManager.getBaseParams(
                        MaintainParamsManager.queryMaintainApprovalParams(datumDataId, datumType)).toString())
                .build()
                .postRaw()

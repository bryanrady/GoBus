...
import com.hxd.gobus.mvp.model.MaintainApprovalModel;
 ...
 */
@ActivityScope
public class MaintainApprovalPresenter extends BasePresenter<IMaintainApprovalContract.View,MaintainApprovalModel> {
    ...
    @Inject
    public MaintainApprovalPresenter(){

    }

    public void queryMaintainApprovalData(){
        if (mDatumDataId != null && !TextUtils.isEmpty(mDatumType)){
            getView().showLoading();
            getModel().queryMaintainApprovalData(mDatumDataId,mDatumType).subscribe(new ObserverImpl() {
                ...
                protected void handleResponse(String response) {
                    ...
                    if (maintain != null && mPositionFlag != null && mSource != 0){
                        String dataId = maintain.getUpkeepApplyId().toString();
                        if (!TextUtils.isEmpty(dataId)){
                            getAttachList(dataId);
                        }
                        if(mSource == BaseConfig.SOURCE_LOOK_DETAIL){
                            if (mDatumDataId != 0 && !TextUtils.isEmpty(mDatumType)){
                                getNextReviewer(mSource,mPositionFlag,maintain,mDatumDataId,mDatumType);
                            }
                        }else{
                            getView().initMaintainData(mSource,mPositionFlag,maintain);
                        }
                    }
                }
    ...
    private void getNextReviewer(int source, String positionFlag, Maintain maintain, int datumDataId, String datumType){
        getView().showLoading();
            ...
            protected void handleResponse(String response) {
                ...
                if (jsonToObject != null){
                    ...
                    if (nextReviewerList != null && nextReviewerList.size()>0){
                        List<DatumDetail> datumDetailInfoList = maintain.getDatumDetailInfoList();
                        if (datumDetailInfoList != null && datumDetailInfoList.size() > 0){
                            ...
                            maintain.setDatumDetailInfoList(datumDetailInfoList);
                            getView().initMaintainData(source,positionFlag,maintain);
                        }
                    }
                }
            }
